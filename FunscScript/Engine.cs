...
using static FunscScript.Core.FunscScriptParser;
...
namespace FunscScript
{
    public static class Engine
    {
        ...
        /// <summary>
        /// Converts a .net value into type that is compatible with FunscScript
        /// </summary>
        ...
        /// <param name="asFunscScriptLiteral">Format as FunscScript literal</param>
        ...
        public static void Format(StringBuilder sb, object val, string format = null,
            bool asFunscScriptLiteral = false,
            bool asJsonLiteral = false)
        {


            Format("", sb, val, format, asFunscScriptLiteral, asJsonLiteral, true);
        }
        ...
        {
            var sb = new StringBuilder();
            Format("", sb, val, format, asFunscScriptLiteral, asJsonLiteral, false);
            return sb.ToString();
        }
        static void Format(String indent, StringBuilder sb, object val,
            string format,
            bool asFunscScriptLiteral,
            bool asJsonLiteral, bool adaptiveLineBreak)
        {
            ...
            if (val is ByteArray)
            {
                if (asFunscScriptLiteral || asFunscScriptLiteral)
                    sb.Append("");
            ...
            if (val is FsList)
            {
                ...
                {
                    var test = TestFormat(val, format, asFunscScriptLiteral, asJsonLiteral);
                ...
                if (list.Length > 0)
                {
                    if (useLineBreak)
                        ...
                        sb.Append($" ");
                    Format($"{indent}{TAB}", sb, list[0], format, asFunscScriptLiteral, asJsonLiteral, adaptiveLineBreak);
                    for (int i = 1; i < list.Length; i++)
                    {
                        if (useLineBreak)
                            ...
                            sb.Append($", ");
                        Format($"{indent}{TAB}", sb, list[i], format, asFunscScriptLiteral, asJsonLiteral, adaptiveLineBreak);
                    }
                }
                if (useLineBreak)
                    sb.Append($"\n{indent}]");
            ...
            if (val is KeyValueCollection)
            {
                ...
                if (pairs.Count > 0)
                {
                    ...
                    if (useLineBreak)
                        ...
                        sb.Append($"\"{pair.Key}\":");
                    Format($"{indent}{TAB}", sb, pair.Value, format, asFunscScriptLiteral, asJsonLiteral, adaptiveLineBreak);
            ...
            if (val is DateTime)
            {
                if (asJsonLiteral || asFunscScriptLiteral)
                    sb.Append("\"");
                if (format == null)
        ...
        public static object Evaluate(string expression, IFsDataProvider provider, object vars, ParseMode mode)
        {
            ...
            var serrors = new List<FunscScriptParser.SyntaxErrorData>();
            ExpressionBlock exp;
            switch (mode)
            {
                case ParseMode.Standard:
                    exp = FunscScriptParser.Parse(provider, expression, serrors);
                    break;
                case ParseMode.SpaceSeparatedList:
                    return FunscScriptParser.ParseSpaceSepratedList(provider, expression, serrors);
                case ParseMode.FsTemplate:
                    exp = FunscScriptParser.ParseFsTemplate(provider, expression, serrors);
                    break;
                default:    
                    exp = null;
                    break;
