...
namespace Walya.Test
{
    public class TestErrorReporting
    {
        ...
        public void TestSyntaxErrorMissingOperand()
        {
            ...
            try
            {
                //WalyaRuntime.Evaluate(exp, new { f = new Func<int, int>((x) => { throw new Exception("internal"); }) });
                WalyaRuntime.EvaluateWithVars(exp, new
                {
        ...
        public void TestLambdaErrorMemberAccessError()
        {
            ...
            {
                //WalyaRuntime.Evaluate(exp, new { f = new Func<int, int>((x) => { throw new Exception("internal"); }) });
                WalyaRuntime.EvaluateWithVars(exp, new { f = new Func<int, int>((x) =>
                {
                    throw new Exception(msg);
                })});
                throw new Exception("No error");
