...
using static Walya.Core.WalyaParser;
...
namespace Walya
{
    public static class Engine
    {
        ...
        /// <summary>
        /// Converts a .net value into type that is compatible with Walya
        /// </summary>
        ...
        /// <param name="asWalyaLiteral">Format as Walya literal</param>
        ...
        public static void Format(StringBuilder sb, object val, string format = null,
            bool asWalyaLiteral = false,
            bool asJsonLiteral = false)
        {


            Format("", sb, val, format, asWalyaLiteral, asJsonLiteral, true);
        }
        ...
        {
            var sb = new StringBuilder();
            Format("", sb, val, format, asWalyaLiteral, asJsonLiteral, false);
            return sb.ToString();
        }
            ...
            bool asJsonLiteral, bool adaptiveLineBreak)
        {
            ...
            if (val is ByteArray)
            {
                if (asWalyaLiteral || asWalyaLiteral)
                    sb.Append("");
            ...
            if (val is FsList)
            {
                ...
                {
                    var test = TestFormat(val, format, asWalyaLiteral, asJsonLiteral);
                ...
                if (list.Length > 0)
                {
                    if (useLineBreak)
                        ...
                        sb.Append($" ");
                    Format($"{indent}{TAB}", sb, list[0], format, asWalyaLiteral, asJsonLiteral, adaptiveLineBreak);
                    for (int i = 1; i < list.Length; i++)
                    {
                        if (useLineBreak)
                            ...
                            sb.Append($", ");
                        Format($"{indent}{TAB}", sb, list[i], format, asWalyaLiteral, asJsonLiteral, adaptiveLineBreak);
                    }
                }
                if (useLineBreak)
                    sb.Append($"\n{indent}]");
            ...
            if (val is KeyValueCollection)
            {
                ...
                if (pairs.Count > 0)
                {
                    ...
                    if (useLineBreak)
                        ...
                        sb.Append($"\"{pair.Key}\":");
                    Format($"{indent}{TAB}", sb, pair.Value, format, asWalyaLiteral, asJsonLiteral, adaptiveLineBreak);
            ...
            if (val is DateTime)
            {
                if (asJsonLiteral || asWalyaLiteral)
                    sb.Append("\"");
                if (format == null)
        ...
        public static object Evaluate(string expression, IFsDataProvider provider, object vars, ParseMode mode)
        {
            ...
            var serrors = new List<WalyaParser.SyntaxErrorData>();
            ExpressionBlock exp;
            switch (mode)
            {
                case ParseMode.Standard:
                    exp = WalyaParser.Parse(provider, expression, serrors);
                    break;
                case ParseMode.SpaceSeparatedList:
                    return WalyaParser.ParseSpaceSepratedList(provider, expression, serrors);
                case ParseMode.FsTemplate:
                    exp = WalyaParser.ParseFsTemplate(provider, expression, serrors);
                    break;
                default:    
                    exp = null;
                    break;
