...
namespace Walya.Functions.Misc
{
    ...
    public class LogFunction : IFsFunction
    {
        ...
        public object Evaluate(IFsDataProvider parent, IParameterList pars)
        {
            ...
            Fslogger.DefaultLogger.WriteLine($"Walya: Evaluating {tag}");
            try
            {
                var res = pars.GetParameter(parent, 0);
                if (output)
                {
                    Fslogger.DefaultLogger.WriteLine($"Walya: Result{tag}:\n{(res == null ? "<null>" : res.ToString())}");
            ...
            {
                Fslogger.DefaultLogger.WriteLine($"Walya: Error evaluating {tag}");
                var thisEx = ex;
                while (thisEx != null)
                {
