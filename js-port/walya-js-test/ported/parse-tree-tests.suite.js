...
const { WalyaParser, ParseNodeType } = require('../../walya-js/src/parser/walya-parser');
const { colorParseTree } = require('../../walya-js/src/walya');
...
function run() {
  ...
  runCase(suite, 'ColorParseTreeSegments', () => {
    ...
    const { parseNode } = WalyaParser.parse(provider, expression);
