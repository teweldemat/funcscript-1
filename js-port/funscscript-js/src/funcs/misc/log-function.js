...
class LogFunction extends BaseFunction {
  ...
  evaluate(provider, parameters) {
    ...
    console.log(`FunscScript: Evaluating ${messageTag}`);
    try {
      const result = helpers.ensureTyped(expression);
      if (output) {
        console.log(`FunscScript: Result ${messageTag}:`);
        console.log(helpers.valueOf(result));
      } else {
        console.log(`FunscScript: Done ${messageTag}`);
      }
      return result;
    } catch (error) {
      console.error(`FunscScript: Error evaluating ${messageTag}`);
      console.error(error);
      throw error;
    }
  }
