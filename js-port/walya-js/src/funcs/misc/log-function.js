...
class LogFunction extends BaseFunction {
  ...
  evaluate(provider, parameters) {
    ...
    console.log(`Walya: Evaluating ${messageTag}`);
    try {
      const result = helpers.ensureTyped(expression);
      if (output) {
        console.log(`Walya: Result ${messageTag}:`);
        console.log(helpers.valueOf(result));
      } else {
        console.log(`Walya: Done ${messageTag}`);
      }
      return result;
    } catch (error) {
      console.error(`Walya: Error evaluating ${messageTag}`);
      console.error(error);
      throw error;
    }
  }
