...
const { Engine } = require('../funcscript-js/src/funcscript');
const { evaluate, DefaultFsDataProvider, FSDataType } = Engine;
const { ensureTyped, typeOf, valueOf } = require('../funcscript-js/src/core/value');
...
function run() {
  ...
  const os = require('os');
  const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), 'funcscript-'));
