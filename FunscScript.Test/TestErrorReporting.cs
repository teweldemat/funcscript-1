...
namespace FunscScript.Test
{
    public class TestErrorReporting
    {
        ...
        public void TestSyntaxErrorMissingOperand()
        {
            ...
            try
            {
                //FunscScriptRuntime.Evaluate(exp, new { f = new Func<int, int>((x) => { throw new Exception("internal"); }) });
                FunscScriptRuntime.EvaluateWithVars(exp, new
                {
        ...
        public void TestLambdaErrorMemberAccessError()
        {
            ...
            {
                //FunscScriptRuntime.Evaluate(exp, new { f = new Func<int, int>((x) => { throw new Exception("internal"); }) });
                FunscScriptRuntime.EvaluateWithVars(exp, new { f = new Func<int, int>((x) =>
                {
                    throw new Exception(msg);
                })});
                throw new Exception("No error");
