...
namespace FunscScript.Test
{
    public class Syntax2
    {
        ...
        public void StringInterpolationBasic()
        {
            ...
            var res = FunscScriptRuntime.Evaluate(p, @"f'y={x+2}'");
        ...
        public void StringDoubleEscapeBug()
        {
            ...
            var p = new KvcProvider(new ObjectKvc(new { x = 100 }),
                new DefaultFsDataProvider());
            var res = FunscScriptRuntime.Evaluate(p, exp);
        ...
        public void NullSafeGetMemberNullValue()
        {
            ...
            var res = FunscScriptRuntime.Evaluate(p, @"x?.y");
        ...
        {
            var p = new DefaultFsDataProvider();
            var res = FunscScriptRuntime.Evaluate(p, @"{ x:{y:5}; return x?.y}");
        ...
        {
            var p = new DefaultFsDataProvider();
            var res = FunscScriptRuntime.Evaluate(p, @"x?!(x*200)");
            Assert.AreEqual(null, res);
        }
        ...
        {
            var p = new DefaultFsDataProvider();
            var res = FunscScriptRuntime.Evaluate(p, @"{ x:5; return x?!(x*200)}");
        ...
        {
            var p = new DefaultFsDataProvider();
            var res = FunscScriptRuntime.Evaluate(p, @"[4,5,6][1]");
            Assert.AreEqual(5, res);
        }
        [Test]
        ...
        {
            var p = new DefaultFsDataProvider();
            var res = FunscScriptRuntime.Evaluate(p, @"{x:[4,5,6];return x[1]}");
            Assert.AreEqual(5, res);
        }
        ...
        public void TestFSTemplate1()
        {
            var template = "abc";
            var expeced = "abc";
            var res = FunscScriptRuntime.Evaluate(template, new DefaultFsDataProvider(), null, FunscScriptRuntime.ParseMode.FsTemplate);
        ...
        //{
        //    var expr = "[[1][2]]";
        //    Assert.Throws<SyntaxError>(()=>FunscScriptRuntime.Evaluate(expr));
