...
namespace FunscScript.Test
{
    ...
    public class TestSet2
    {
        ...
        public void CallingNoneFunction()
        {
            ...
            Assert.Throws(typeof(EvaluationException), () =>
            {
                ...
                FunscScriptRuntime.Evaluate(g,"3(4,5)");
            });
        }
        [Test]
        public void DuplicateKeyInCollection()
        {
            ...
            {
                FunscScriptRuntime.Evaluate("{a:5;a:6;return 5;}");
            });
        }
    ...
    public class Tests
    {
        ...
        public static object AssertSingleResult(string expStr)
        {
            ...
            var ret = FunscScriptRuntime.Evaluate(p, expStr);
            Assert.AreEqual(ret, FunscScriptRuntime.NormalizeDataType(ret));
            return ret;
        }
        public static object AssertSingleResultType(string expStr, System.Type type)
        {
            var p = new DefaultFsDataProvider();
            var res = FunscScriptRuntime.Evaluate(p, expStr);
            Assert.AreEqual(res, FunscScriptRuntime.NormalizeDataType(res));
        ...
        public void TestOverflow()
        {
            ...
            {
                FunscScriptRuntime.Evaluate($"{long.MaxValue}0");
            });
        }
        [Test]
        [TestCase("12e-")]
        public void TestInvalid(string exp)
        {
            Assert.Throws<Error.SyntaxError>(() =>
            {
                FunscScriptRuntime.Evaluate(exp);
            });
        }
        [Test]
...
@"{return (x)=>3;}(3)", 3)]
        ...
        public void TestSpread()
        {
            ...
            var expected = FunscScriptRuntime.Evaluate(null, "{'a':1,'b':5,'c':8}");
        ...
        {
            var g = new DefaultFsDataProvider();
            var res = FunscScriptRuntime.Evaluate(g, "Select({'a':1,'b':5,'c':8},{'a':null,'b':null})");
        ...
        {
            var g = new DefaultFsDataProvider();
            var res = FunscScriptRuntime.Evaluate(g, "Select({a:3,b:4},{a,c:5})");
        ...
        public void MapNull()
        {
            ...
            Assert.IsNull(FunscScriptRuntime.Evaluate("null map (x)=>x"));
            Assert.IsNull(FunscScriptRuntime.Evaluate("y map (x)=>x"));
        }
        ...
        {
            Assert.AreEqual(expected, FunscScriptRuntime.Evaluate(exp));
        }
        ...
        {
            var exp = "((X)=>X)('t')";
            Assert.AreEqual("t", FunscScriptRuntime.Evaluate(exp));
        }
        [Test]
        public void EmptyKeyValueCollection()
        {
            var exp = "{}";
            var res = FunscScriptRuntime.Evaluate(exp) as KeyValueCollection;
...
@"{
    ...
    return fib(3);
        ...
        public void TestFormatAsJson()
        {
            var exp =
                @"{x:5,y:6}";
            var obj = FunscScriptRuntime.Evaluate(exp);
            var sb = new StringBuilder();
            FunscScriptRuntime.Format(sb,obj,asJsonLiteral:true);
